...
def load_what_we_need(model_training_output_dir, use_folds, checkpoint_name):
    ...
    """                                                   
    network = MedNeXt(in_channels=2, n_channels=32, n_classes=2, exp_r = 2, kernel_size=3,
                                  deep_supervision=False, do_res=True,
                                  do_res_up_down=True,
                                  block_counts = [2,2,2,2,2,2,2,2,2],
                                  # block_counts=[3, 4, 8, 8, 8, 8, 8, 4, 3],
                                  checkpoint_style='outside_block', dim='3d', grn=True)#outside_block##
    print(network)
                ...
                if perform_everything_on_gpu:
                    try:
                        for params in parameters:
                            network.load_state_dict(params,strict=False)###################################################################
