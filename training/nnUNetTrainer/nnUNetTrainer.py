...
from nnunetv2.training.nnUNetTrainer.MedNextV2 import MedNeXt
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        #self.grad_scaler = None #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!without autocast
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            """
            self.network = MedNeXt(in_channels=2, n_channels=32, n_classes=2, exp_r = 2, kernel_size=3,
                                  deep_supervision=True, do_res=True,
                                  do_res_up_down=True,
                                  block_counts = [2,2,2,2,2,2,2,2,2], 
                                  #block_counts=[3, 3, 4, 4, 4, 4, 4, 3, 3], 
                                  checkpoint_style='outside_block', dim='3d', grn=True).to(self.device)#outside_block#################################################  
    ...
    def _build_loss(self):
        ...
        #weights = np.array([1 / (8 ** i) for i in range(len(deep_supervision_scales))]) 
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(self.network.parameters(), lr=self.initial_lr, betas=(0.9, 0.99), weight_decay=self.weight_decay)  
    ...
    def get_dataloaders(self):
        ...
        print('training pipeline:',tr_transforms)
        # validation pipeline
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            for num_out in range(len(output)):
              if torch.any(torch.isnan(output[num_out])):
                print('sth. NAN in output%s'%num_out)
            #print('sth. output')
            # del data
            l = self.loss(output, target)
            if torch.any(torch.isnan(l)):
              print('sth. NAN in loss')
              sys.exit()
        """
        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!without autocast
        output = self.network(data)
        for num_out in range(len(output)):
          if torch.any(torch.isnan(output[num_out])):
            print('sth. NAN in tra_output%s'%num_out)
        del data
        l = self.loss(output, target)
        if torch.any(torch.isnan(l)):
          print('sth. NAN in tra_loss')
          sys.exit()
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!without autocast
        output = self.network(data)
        #print('predicted_segmentation_onehot:',(output[0].dtype))
        if torch.any(torch.isnan(output[0])):
          print('sth. NAN in val_output')
        del data
        l = self.loss(output, target)
        if torch.any(torch.isnan(l)):
          print('sth. NAN in val_loss')
        ...
        if self.label_manager.has_ignore_label:
            ...
            mask = None
        #print('predicted_segmentation_onehot:',(type(predicted_segmentation_onehot)))
